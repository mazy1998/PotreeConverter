...
void buildHierarchy(Indexer* indexer, Node* node, shared_ptr<Buffer> points, int numPoints, int depth = 0) {
	...
	auto gridIndexOf = [&points, bpp, scale, offset, min, size, counterGridSize](int64_t pointIndex) {
		...
		ix = std::max(0ll, std::min(static_cast<long long>(ix), static_cast<long long>(counterGridSize) - 1));
		iy = std::max(0ll, std::min(static_cast<long long>(iy), static_cast<long long>(counterGridSize) - 1));
		iz = std::max(0ll, std::min(static_cast<long long>(iz), static_cast<long long>(counterGridSize) - 1));
